...
<body>
	<%--
		...
		JSP 페이지당 하나의 pageContext객체 생성된다.
		같은 페이지에서 서로 값을 공유할 수 있으면,
		주요 기능은 다른 기본 내장 객체를 호출하거나, 페이지 흐름을 제어할 때 사용
		- 다른 기본 내장 객체 호출
			--  getRequest(), getResponse(), getOut, getSession()
				...
				** request와 같은 방법
				...
				데이터 호출 : pageContext.setAttribute("이름");
				...
				** 속성을 이용해서 데이터 저장 및 호출하는 내장 객체들
				...
				생명주기 : pageContext < request < session < application
					 	 ...
					 	 pageContext : 해당페이지를 벗어나면 정보도 사라진다.
					 	 request : response가 되면 사라진다. 		
					 	 session : 해당 브라우저가 종료되면 저장 정보도 사라진다.
					 	 			(예외적으로 시간을 지정할 수 있다.)
					 	 			보통은 로그인 처리할 때 사용
					 	 application : 해당 프로그램이 종료될 때까지 정보가 사라지지 않는다.
					 	 				보통 외부 환경 정보를 가지고 있다.
	 ...
	 %>
		...
		<h2>application : <%= application.getAttribute("name") %></h2>
	 	...
	 	<h1>EL(표현언어))</h1>
	 	<h2>이름 : ${name }</h2> <!-- pageContext를 주석처리하면 둘리가 나온다 -->
