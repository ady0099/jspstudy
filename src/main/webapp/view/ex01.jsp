		...
		<%--
			JSP : Java Server Page
		 	...
		 	JSP : 구성
		 		1. 지시어(디렉티브) : <%@지시어 %>
		 			--> <%@ page %>, <%@ include %>, <%@ taglib %>
				2. 스크립트요소 : HTML 안에서 실제 자바코드를 사용할 수 있도록 만드는 요소
					1) 선언부(<%! 내용 %>) 			: 변수 선언이나 메소드 선언할 때 사용
					2) 스크립트릿(<% 자바코드 %>)		: 자바코드를 코딩할 때 사용
					3) 표현식(<%= %>)				: 변수값이나 메서드의 결과 등 저장되어있는 결과값 출력	
					** 현재는 표현식 대신 표현 언어(EL과 JSTL을 사용한다.) 
					
			** 주의 사항 : 선언부, 스크립트릿, 표현식은 겹쳐서(포함해서 사용할 수 없다.
		 	...
		 	1. 지시어 : <%@ page %>, <%@ include %>, <%@ taglib %>
		 		1) page 지시어 : 보통 해당 문서의 맨 앞에 나온다.
		 		   ...
		 		   - isErrorPage : 해당 페이지가 오류를 처리할 수 있는 페이지이면 true, 아니면 false(기본값 false)
		 		   ...
		 		   3) include 지시어 : HTML의 iframe과 비슷하다
		 		   	...
		 		   	** include는 지시어를 사용하는 방법과 include 액션태그를 사용하는 방법 두 가지다.
